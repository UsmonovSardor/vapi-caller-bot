...
function getSession(id) {
  if (!sessions.has(id)) sessions.set(id, { waitingFor: 'waiting_prompt', prompt: '', phone: '' });
...
async function call(phone, prompt) {
  try {
...
app.post('/webhook', async (req, res) => {
  ...
  if (['/start','/menu'].includes(text)) {
    sessions.set(chatId, { waitingFor: 'waiting_prompt', prompt: '', phone: '' });
  ...
  if (text === '🧠 Prompt') {
    s.waitingFor = 'waiting_prompt'; sessions.set(chatId, s);
    await send(chatId, '🧠 Prompt yuboring yoki pastdagi 5 ta tayyor promptdan birini tanlang.', promptKb);
    return;
  }
  if (text === '📞 Nomer') {
    s.waitingFor = 'waiting_phone'; sessions.set(chatId, s);
    await send(chatId, '📞 Endi telefon raqam yuboring.\nFormat: +998901234567', rmKb);
    return;
  }
  if (s.waitingFor === 'waiting_prompt') {
    if (text.length < 5) { await send(chatId, '❌ Prompt juda qisqa. Kamida 5 belgi kiriting.', promptKb); return; }
    s.prompt = text; s.waitingFor = 'waiting_phone'; sessions.set(chatId, s);
    await send(chatId, '✅ Prompt saqlandi!\n\n📞 Endi telefon raqam yuboring.\nFormat: +998901234567', rmKb);
    return;
  }
  if (s.waitingFor === 'waiting_phone') {
    const phone = clean(text);
    if (!phoneRe.test(phone)) { await send(chatId, "❌ Telefon raqam noto'g'ri.\nTo'g'ri format: +998901234567", rmKb); return; }
    s.phone = phone;
    if (!s.prompt) {
      s.waitingFor = 'waiting_prompt'; sessions.set(chatId, s);
    ...
    const result = await call(phone, s.prompt);
    sessions.set(chatId, { waitingFor: 'waiting_prompt', prompt: '', phone: '' });
...
app.listen(PORT, async () => {
  ...
  if (WEBHOOK_URL) {
    try {
      ...
      console.log('Webhook set:', r.data.description);
